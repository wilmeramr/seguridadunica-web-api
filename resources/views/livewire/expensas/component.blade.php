<div class="row sales layout-top-spacing">
                ...
                <tbody>
                    ...
                    <tr >
                        <td class="text-center">
                            <h6>{{$expensa->lot_name}}</h6>
                        </td>



                         <td class="text-center">
                            <h6>{{$expensa->exp_name}}</h6>
                        </td>

                        <td>

                            @if (\Str::contains(strtolower($expensa->exp_doc_url),'http'))
                            <a target="_blank" href="{{$expensa->exp_doc_url}}"> <h6>Ir</h6></a>
                        ...
                        <td class="text-center">
                            <h6>{{$expensa->exp_link_pago}}</h6>
                        </td>

                        <td class="text-center">
                            <h6>{{$expensa->updated_at}}</h6>
                        </td>

                        <td class="text-center">
                            <a href="javascript:void(0)"
                            wire:click.prevent="show({{$expensa->exp_id}})"
                                class="btn bnt-dark mtmobile" title="Ver">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-eye"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>
                            </a>

                            <a href="javascript:void(0)" onclick="Confirm({{$expensa->exp_id}})" class="btn bnt-dark " title="Delete">
    ...
    @include('livewire.expensas.form')
    @include('livewire.expensas.details')
    ...
    document.getElementById('img_foto').style.width = 0;
        ...
        document.addEventListener('DOMContentLoaded',function(){
           ...
           $('#theModal').on('shown.bs.modal',function(){
            ...
            window.livewire.on('exp-added',msg=>{
                $('#theModal').modal('hide');
                noty(msg);
            });

            window.livewire.on('exp-updated',msg=>{
                $('#theModal').modal('hide');
                noty(msg);
            });

            window.livewire.on('exp-deleted',msg=>{
                noty(msg);
            });
        ...
        function Confirm(id){
            swal({
                title: "CONFIRMAR",
                text: "¿COFIRMAS LA ELIMINACIÓN DEL REGISTRO?",
                type: 'warning',
